...
class TradingBot:
    def __init__(self, symbol, timeframe, limit, start_time=None, end_time=None):
        ...
        self.default_strategy = ripsterClouds
    ...
    def __run_bot(self, env):
            ...
            if len(stop_loss_ids) > 0:
                for order_id in stop_loss_ids:
                    if env == "TEST":
                        ...
                        result = self.broker.delete_existing_order(order_id)
                        self.tg_bot.send_message(f"{env}: Stop Loss removed:"
                                                 f"\nAPI response: {result}")
        ...
        def handle_stop_loss(payload):
            if env == "TEST":
                ...
                result_dict = json.loads(result)  # Convert the JSON string to a dictionary
                stop_loss_ids.append({'orderId': result_dict['orderId']})
            return result
                ...
                if payloads:
                    try:
                        for payload in payloads:
                            ...
                            self.tg_bot.send_message(f"{env}: Position posted:"
                                                     f"\nSide: {payload['side']}"
                                                     f"\nType: {payload['type']}"
                                                     f"\nTime: {time.ctime()}"
                                                     f"\nAPI response: {result}")
...
def main():
    ...
    #eth.default_strategy = ripsterClouds
